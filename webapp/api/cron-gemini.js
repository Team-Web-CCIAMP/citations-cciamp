...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const validQueries = queries.filter(q => q.label && q.prompt);

    // On interroge Gemini par petits groupes en parallèle plutôt qu'une par
    // une : le plan gratuit de Vercel limite chaque fonction à 10 secondes,
    // et 20 appels strictement séquentiels seraient bien trop lents.
    const BATCH_SIZE = 5;
    const newEntries = [];
    for (let i = 0; i < validQueries.length; i += BATCH_SIZE) {
      const batch = validQueries.slice(i, i + BATCH_SIZE);
      const results = await Promise.all(batch.map(q => queryGemini(q.prompt)));
      batch.forEach((q, j) => {
    ...
    const errors = newEntries.filter(e => e.status === 'error').length;
    res.status(200).json({ ok: true, added: newEntries.length, errors, history: finalHistory });
  } catch (err) {
...
// --- Appel Gemini avec recherche web (avec 1 nouvelle tentative si quota momentanément dépassé) ---
async function queryGemini(prompt, attempt = 0) {
  ...
  if (r.status === 429 && attempt < 1) {
    ...
    return queryGemini(prompt, attempt + 1);
  }
